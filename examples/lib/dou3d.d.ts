...
declare namespace dou3d {
    ...
    namespace ShaderLib {
        ...
        const normalPassEnd_fs = "void main\r\r{\rgl_FragColor=vec4\rnormal,1.0\r;\r}";
        const pickPass_fs = "uniform vec4 uniform_ObjectId;\rvoid main\r\r{\rgl_FragColor=uniform_ObjectId;\r}";
        const pickPass_vs = "attribute vec3 attribute_position;\rattribute vec4 attribute_color;\rattribute vec2 attribute_uv0;\runiform mat4 uniform_ModelMatrix;\runiform mat4 uniform_ViewMatrix;\runiform mat4 uniform_ProjectionMatrix;\rvoid main\rvoid\r{\rgl_Position=uniform_ProjectionMatrix*uniform_ViewMatrix*uniform_ModelMatrix*vec4\rattribute_position,1.0\r;\r}";
        const positionEndPass_fs = "varying vec4 varying_position;\rvoid main\r\r{\rgl_FragColor=vec4\rvarying_position.xyz,1.0\r;\r}";
        const positionEndPass_vs = "varying vec4 varying_position;\rvoid main\r\r{\rgl_Position=uniform_ProjectionMatrix*outPosition;\rvarying_position=gl_Position.xyzw;\r}";
        const shadowMapping_fs = "uniform sampler2D shadowMapTexture;\runiform vec4 uniform_ShadowColor;\rvarying vec4 varying_ShadowCoord;\rvoid main\r\r{\rvec3 shadowColor=vec3\r1.0,1.0,1.0\r;\rfloat offset=uniform_ShadowColor.w;\rvec2 sample=varying_ShadowCoord.xy/varying_ShadowCoord.w*0.5+0.5;\rif\rsample.x>=0.0 && sample.x<=1.0 && sample.y>=0.0 && sample.y<=1.0\r{\rvec4 sampleDepth=texture2D\rshadowMapTexture,sample\r.xyzw;\rfloat depth=varying_ShadowCoord.z;\rif\rsampleDepth.z !=0.0\r{\rif\rsampleDepth.z<depth-offset\r{\rshadowColor=uniform_ShadowColor.xyz;\r}\r}\r}\rdiffuseColor.xyz=diffuseColor.xyz*shadowColor;\r}";
        const shadowMapping_vs = "uniform mat4 uniform_ShadowMatrix;\runiform mat4 uniform_ModelMatrix;\rvarying vec4 varying_ShadowCoord;\rvoid main\r\r{\rvarying_ShadowCoord=uniform_ShadowMatrix*uniform_ModelMatrix*vec4\re_position,1.0\r;\r}";
        const shadowPass_fs = "uniform sampler2D diffuseTexture;\rvec4 diffuseColor;\rvarying vec2 varying_uv0;\rvarying vec4 varying_color;\rvarying vec4 varying_pos;\rvoid main\r\r{\rdiffuseColor=varying_color;\rif\rdiffuseColor.w==0.0\r{\rdiscard;\r}\rdiffuseColor=texture2D\rdiffuseTexture,varying_uv0\r;\rif\rdiffuseColor.w<=0.3\r{\rdiscard;\r}\rgl_FragColor=vec4\rvarying_pos.zzz,1.0\r;\r}";
        const shadowPass_vs = "attribute vec3 attribute_position;\rattribute vec4 attribute_color;\rattribute vec2 attribute_uv0;\runiform mat4 uniform_ModelMatrix;\runiform mat4 uniform_ViewMatrix;\runiform mat4 uniform_ProjectionMatrix;\rvarying vec2 varying_uv0;\rvarying vec4 varying_color;\rvarying vec4 varying_pos;\rvoid main\rvoid\r{\rmat4 mvMatrix=mat4\runiform_ViewMatrix*uniform_ModelMatrix\r;\rvarying_color=attribute_color;\rvarying_uv0=attribute_uv0;\rvarying_pos=uniform_ProjectionMatrix*uniform_ViewMatrix*uniform_ModelMatrix*vec4\rattribute_position,1.0\r;\rgl_Position=varying_pos;\r}";
    }
}
declare namespace dou3d {
    /**
